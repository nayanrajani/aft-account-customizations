  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
        ...
        "Resource": "arn:aws:s3:::${local.cloudwatch_logs_export_bucket}/*"
    },
    {
...
resource "aws_lambda_function" "log_exporter" {
  ...
  environment {
    variables = {
      S3_BUCKET   = local.cloudwatch_logs_export_bucket,
      AWS_ACCOUNT = local.log_archive_account_id # used for prefix in s3 bucket
    }
  }
}
...
resource "aws_cloudwatch_event_rule" "log_exporter" {
  ...
  schedule_expression = local.vpn_log_export_rate
  is_enabled          = local.enable_vpn_log_export
}
